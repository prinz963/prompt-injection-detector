...
<body>
    ...
    async function analyze() {
        ...
        div.innerHTML = `
            ...
            ${!safe ? `<div style="margin-bottom:10px">Attack type: <b>${data.attack_category}</b></div>` : ""}
            ...
            <div class="row"><span>Risk Score</span>
                <b>${data.risk_score}/2 detectors fired</b></div>
            <div class="row"><span>Response time</span>
                <b>${data.response_time_ms}ms</b></div>
        `;
...
@app.route("/health")
def health():
    return jsonify({"status": "ok", "detectors": 2})
